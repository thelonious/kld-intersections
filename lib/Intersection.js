...
(function (global, factory) {
...
}(this, function (Point2D, Vector2D, Matrix2D, Polynomial) {
    ...
    */
    Intersection.intersectShapes = function (shape1, shape2) {
        ...
        if (!composedShapeMethods) {
            composedShapeMethods = {};
            composedShapeMethods[IPTYPE.PATH] = Intersection.intersectPathShape;
            composedShapeMethods[IPTYPE.POLYLINE] = Intersection.intersectLinesShape;
            composedShapeMethods[IPTYPE.POLYGON] = Intersection.intersectLinesShape;
            composedShapeMethods[IPTYPE.RECT] = Intersection.intersectLinesShape;
            composedShapeMethods[IPTYPE.ROUNDRECT] = Intersection.intersectPathShape;
        }
        ...
        if (ip1 !== null && ip2 !== null) {
            ...
            if (method = composedShapeMethods[ip1.type]) {
                result = method(ip1, ip2);
            }
            else if (method = composedShapeMethods[ip2.type]) {
                result = method(ip2, ip1);
            }
            else {
                var params;

                if (ip1.type < ip2.type) {
                    method = "intersect" + ip1.type + ip2.type;
                    params = ip1.params.concat(ip2.params);
                } else {
                    method = "intersect" + ip2.type + ip1.type;
    ...
    */
    Intersection.intersectPathShape = function (path, shape) {
        ...
        for (var inter, i = 0; i < pathParams.length; i++) {
            inter = Intersection.intersectShapes(pathParams[i], shape);
            if (!inter0)
                inter0 = inter;
            if (previnter) {
    ...
    /** 
        ...
        @returns {Intersection}
    */
    Intersection.intersectLinesShape = function (lines, shape, closed) {
        ...
        var intersectLine = function (i1, i2) {
            ...
            inter = Intersection.intersectShapes(ip, shape);
    ...
    /** 
        ...
        @param {Point2D} c1 - center of first arc ellipse
        @param {Number} rx1
        @param {Number} ry1
        @param {Number} phi1 - in radians
        @param {Number} th11 - in radians
        @param {Number} dth1 - in radians
        @param {Point2D} c2 - center of second arc ellipse
        @param {Number} rx2
        @param {Number} ry2
        @param {Number} phi2 - in radians
        @param {Number} th12 - in radians
        @param {Number} dth2 - in radians
        @returns {Intersection}
    */
    Intersection.intersectArcArc = function (c1, rx1, ry1, phi1, th11, dth1, c2, rx2, ry2, phi2, th12, dth2) {
        var res = Intersection.intersectEllipseEllipse(c1, rx1, ry1, c2, rx2, ry2);
        return Intersection.Utils.removePointsNotInArc(Intersection.Utils.removePointsNotInArc(res, c1, rx1, ry1, phi1, th11, dth1), c2, rx2, ry2, phi2, th12, dth2);
    };
    ...
    /** 
        ...
        @param {Point2D} c1 - arc ellipse center
        @param {Number} rx1 - arc ellipse x radius
        @param {Number} ry1 - arc ellipse y radius
        @param {Number} phi1 - arc ellipse rotation (in radians)
        @param {Number} th1 - arc angle (in radians) of start point on circle
        @param {Number} dth1 - arc angle (in radians) from start to end point on circle
        @param {Point2D} p1
        @param {Point2D} p2
        @param {Point2D} p3
        @returns {Intersection}
    */
    Intersection.intersectArcBezier2 = function (c1, rx1, ry1, phi1, th1, dth1, p1, p2, p3) {
        var res = Intersection.intersectBezier2Ellipse(p1, p2, p3, c1, rx1, ry1);
        return Intersection.Utils.removePointsNotInArc(res, c1, rx1, ry1, phi1, th1, dth1);
    };
    ...
    /** 
        ...
        @param {Point2D} c1 - center of arc ellipse
        @param {Number} rx1
        @param {Number} ry1
        @param {Number} phi1 - in radians
        ...
        @returns {Intersection}
    */
    Intersection.intersectArcBezier3 = function (c1, rx1, ry1, phi1, th1, dth1, p1, p2, p3, p4) {
        var res = Intersection.intersectBezier3Ellipse(p1, p2, p3, p4, c1, rx1, ry1);
        return Intersection.Utils.removePointsNotInArc(res, c1, rx1, ry1, phi1, th1, dth1);
    };
    ...
    /** 
        ...
        @returns {Intersection}
    */
    Intersection.intersectArcCircle = function (c1, rx1, ry1, phi1, th1, dth1, c, r) {
        var res = Intersection.intersectCircleEllipse(c, r, c1, rx1, ry1);
        return Intersection.Utils.removePointsNotInArc(res, c1, rx1, ry1, phi1, th1, dth1);
    };
    ...
    /** 
        ...
        @param {Number} th1 - in radians
        @param {Number} dth1 - in radians
        @param {Point2D} c2 - center of ellipse
        @param {Number} rx2
        @param {Number} ry2
        @returns {Intersection}
    */
    Intersection.intersectArcEllipse = function (c1, rx1, ry1, phi1, th1, dth1, c2, rx2, ry2) {
        var res = Intersection.intersectEllipseEllipse(c1, rx1, ry1, c2, rx2, ry2);
        return Intersection.Utils.removePointsNotInArc(res, c1, rx1, ry1, phi1, th1, dth1);
    };
    ...
    /** 
        ...
        @returns {Intersection}
    */
    Intersection.intersectArcLine = function (c1, rx1, ry1, phi1, th1, dth1, a1, a2) {
        var res = Intersection.intersectEllipseLine(c1, rx1, ry1, a1, a2);
        return Intersection.Utils.removePointsNotInArc(res, c1, rx1, ry1, phi1, th1, dth1);
    };
    ...
    Intersection.intersectEllipseEllipse = function (c1, rx1, ry1, c2, rx2, ry2) {
        ...
        var poly = yPoly;
        var ZEROepsilon = 10 * Math.pow(2, poly.getDegree() + 2) * Math.pow(10, -(15) + Math.floor(Math.log(Math.abs(poly.coefs[0] / poly.coefs[poly.coefs.length - 1])) / Math.LN10));
        ZEROepsilon = ZEROepsilon * Math.SQRT2;
    ...
    Intersection.intersectEllipseLine = function(c, rx, ry, a1, a2) {
        ...
        var ERRF = 1e-14;
        var ZEROepsilon = Math.max(Math.abs(a), Math.abs(b), Math.abs(c)) * ERRF;
    ...
    */
    ...
    /** 
        ...
        @returns {Intersection}
    */
    Intersection.Utils.removePointsNotInArc = function (intersection, c, rx, ry, phi, th1, dth) {
        ...
        for (var i = 0, p, a; i < intersection.points.length; i++) {
            ...
            a = vx.transform(m3).angleBetween(Vector2D.fromPoints(c, p));
            if (dth >= 0) {
                a = (a + 2 * pi2) % pi2;
                if (a <= dth)
                    np.push(p);
    ...
    */
    Intersection.Utils.removeClosePoints = function (points1, points2) {
        ...
        var ERRF = 1e-14;
        var ZEROepsilon = 10 * max * ERRF * Math.SQRT2;
        var j;
