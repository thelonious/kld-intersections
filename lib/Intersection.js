/* eslint-disable camelcase, eqeqeq, no-labels */
/**
 *
 *  Intersection.js
 *
...
let Point2D, Vector2D, Polynomial;
if (typeof module !== "undefined") {
    ({Point2D, Vector2D} = require("kld-affine"));
    ({Polynomial} = require("kld-polynomial"));
}
...
Intersection.intersectPathShape = function(path, shape) {
    ...
    const {length: len} = path.args;

    for (let i = 0; i < len; i++) {
        const segment = path.args[i];
