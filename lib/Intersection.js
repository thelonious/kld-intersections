...
/**
 *  includesPolygonPoint
 *
 *  @param {Array<Point2D>} polygon
 *  @param {Point2D} point
 *  @returns {boolean}
 */
Intersection.includesPolygonPoint = function(polygon, point) {
  var x = point.x,
    y = point.y
 ...
 *  @returns {Intersection}
 */
Intersection.intersectLinePolygon = function(a1, a2, _points) {
  var points = closePolygon(_points)
  var result = new Intersection('No Intersection')
  var length = points.length
  ...
  if (Intersection.includesPolygonLine(points, a1, a2)) result.status = 'Inside'
 ...
 *  @returns {Intersection}
 */
Intersection.intersectPolygonPolyline = function(_points1, points2) {
  var points1 = closePolygon(_points1)
  var result = new Intersection('No Intersection')
  ...
  if (Intersection.includesPolygonPolyline(points1, points2)) result.status = 'Inside'
 ...
 *
 *  @param {Array<Point2D>} polygon
 *  @param {Array<Point2D>} polyline
 *  @returns {boolean}
 */
Intersection.includesPolygonPolyline = function(polygon, polyline) {
  if (polyline.length < 2) return false
  return polyline.slice(0, -1).every((point, i) => Intersection.includesPolygonLine(polygon, point, polyline[i + 1]))
}
 ...
 *  @returns {boolean}
 */
Intersection.includesPolygonLine = function(polygon, p1, p2) {
  return Intersection.includesPolygonPoint(polygon, p1) && Intersection.includesPolygonPoint(polygon, p2)
}
