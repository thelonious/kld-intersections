...
export default class ShapeInfo {
    ...
    static fromObject(obj) {
        ...
        if (missingProperties.length > 0) {
            ...
            throw new TypeError(`Missing required properties for ${type}: ${names}`);
        }
        ...
        if (invalidProperties.length > 0) {
            const names = invalidProperties.map(property => property.name).join(", ");

            if (invalidProperties.length === 1) {
                throw new TypeError(`The following property has an invalid type: ${names}`);
            }
            else {
                throw new TypeError(`The following properties have invalid types: ${names}`);
            }
        }
